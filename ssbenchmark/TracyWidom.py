...
class TracyWidom:
    ...
    def __init__(self, beta=2):
        ...
        print(len(_digits_2))
        print(len(_digits_4))
        y = np.fromstring(digits, dtype = np.float32)
        y = y.astype(float)
        y = y * 1E-6
        #y = np.fromstring(digits, dtype=np.int32, count = -1).astype(float) * 1.e-6
        self.__xlim = (x[1], x[-2])
